...
class AdminRecord(tk.Frame):
    ...
    def _dialog_action(self, dialog, action, record):
        ...
        if action == "edit":
            if self.navigate_to:
                self.navigate_to("edit_record", record=record)
        elif action == "delete":
    ...
    def nav_add_record(self):
        if self.navigate_to:
            self.navigate_to("add_record", admin_id=self.user_id)
    ...
    def delete_record(self, record):
        ...
        if confirm:
            try:
                ...
                if self.firebase_db.delete_record(record_id, admin_id=self.user_id):

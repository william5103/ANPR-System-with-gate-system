...
class AdminAddSecurity(tk.Frame):
    ...
    def build_form(self):
        ...
        for i, (label, key) in enumerate(labels):
            ...
            show = "*" if key == "security_password" else None
    ...
    def save_security(self):
        try:
            ...
            if not data["security_id"] or not data["security_name"] or not data["security_ic"] or not data["security_email"] or not data["security_phone"] or not data["security_address"] or not data["security_password"]:
                self.play_warning_sound()
                messagebox.showerror("Error", "All fields are required.")
                return

            # Validate email format
            if not self.validate_email(data["security_email"]):
                self.play_warning_sound()
                messagebox.showerror("Error", "Invalid email format. Email must contain '@' and '.'")
                return

            # Validate IC format (must be 12 digits)
            security_ic = data.get("security_ic", "").strip()
            ...
            security_phone = data.get("security_phone", "").strip()
            ...
            security_password = data.get("security_password", "").strip()
            ...
            try:
                ...
                hashed_pw = bcrypt.hashpw(data["security_password"].encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
                # Add the security personnel to the database
                self.firebase_db.add_security(
                    security_id=data["security_id"],
                    security_name=data["security_name"],
                    security_ic=data["security_ic"],
                    security_email=data["security_email"],
                    security_phone=data["security_phone"],
                    security_address=data["security_address"],
                    security_password=hashed_pw,
                    admin_id=self.user_id  # Pass admin_id for logging
                )

                self.play_success_sound()
                messagebox.showinfo("Success", "Security personnel added successfully.")
                if self.navigate_to:
                    self.navigate_to("security")
            except Exception as e:
                self.play_warning_sound()
                messagebox.showerror("Error", f"Failed to add security personnel: {e}")
        except Exception as e:
            self.play_warning_sound()
            messagebox.showerror("Error", f"Failed to add security personnel: {e}")

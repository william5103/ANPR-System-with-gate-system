...
class ToggleEntry(ctk.CTkFrame):
    def __init__(self, master, placeholder_text="", show=None, width=400, show_icon_path="Pic/show.png", hide_icon_path="Pic/hide.png", **kwargs):
        ...
        self.show = show
        self.entry = ctk.CTkEntry(self, placeholder_text=placeholder_text, width=width, height=55,
                                  fg_color=ENTRY_FG_COLOR, text_color=ENTRY_TEXT_COLOR, font=ENTRY_FONT, show=show)
        ...
        self.toggle_btn = ctk.CTkButton(self, text="", width=40, height=40,
                                        fg_color="transparent", hover_color="#e0e0e0",image=self.hide_icon_img if not self.visible else self.show_icon_img,
...
class SecuritySetting(ctk.CTkFrame):
    def __init__(self, master, user_id, user_name, user_email='', user_phone='', user_address='', firebase_db=None, *args, **kwargs):
        ...
        if self.firebase_db:
            user_doc = self.firebase_db.get_security_person(self.user_id)
            if user_doc:
                self.user_email = user_doc.get('security_email', self.user_email)
                self.user_phone = user_doc.get('security_phone', self.user_phone)
                self.user_address = user_doc.get('security_address', self.user_address)
    ...
    def build_widgets(self):
        ...
        ctk.CTkLabel(details_card, text="👤 User Details", font=LABEL_FONT, text_color="#1a3e72").pack(anchor="w", padx=20, pady=(16, 0))
        self._info_row(details_card, "User ID:", self.user_id)
        self._info_row(details_card, "Name:", self.user_name)
        ...
        ctk.CTkButton(email_row, text="Update Email", command=self.perform_email_update, fg_color="#1769aa", text_color="white", height=45, font=BUTTON_FONT, width=150).pack(side="left", padx=(12, 0))
        ...
        ctk.CTkButton(phone_row, text="Update Phone", command=self.perform_phone_update, fg_color="#1769aa", text_color="white", height=45, font=BUTTON_FONT, width=150).pack(side="left", padx=(12, 0))

        address_row = ctk.CTkFrame(contact_card, fg_color="white")
        address_row.pack(fill="x", padx=20, pady=(10, 0))
        self.address_entry = ctk.CTkTextbox(address_row, width=300, height=70, fg_color=ENTRY_FG_COLOR, text_color=ENTRY_TEXT_COLOR, font=ENTRY_FONT, border_width=2, border_color="#b0b0b0")
        self.address_entry.insert("1.0", self.user_address)
        self.address_entry.pack(side="left", fill="x")
        ctk.CTkButton(address_row, text="Update Address", command=self.perform_address_update, fg_color="#1769aa", text_color="white", height=45, font=BUTTON_FONT, width=150).pack(side="left", padx=(12, 0))
    ...
    def perform_password_update(self):
        ...
        # Check old password
        user_doc = self.firebase_db.get_security_person(self.user_id)
        stored_hash = user_doc.get('security_password') if user_doc else None
        if not user_doc or not stored_hash or not bcrypt.checkpw(old_pw.encode('utf-8'), stored_hash.encode('utf-8')):
        ...
        def on_tac_success():
            ...
            success = self.firebase_db.update_security(self.user_id, {'security_password': hashed_pw})
            if success:
    ...
    def perform_email_update(self):
        ...
        success = self.firebase_db.update_security(self.user_id, {'security_email': email})
        if success:
    ...
    def perform_phone_update(self):
        ...
        if not phone or not phone.isdigit() or len(phone) not in (10, 11):
            ...
            return
        success = self.firebase_db.update_security(self.user_id, {'security_phone': phone})
        if success:
    ...
    def perform_address_update(self):
        ...
        if not address:
            ...
            return
        success = self.firebase_db.update_security(self.user_id, {'security_address': address})
        if success:

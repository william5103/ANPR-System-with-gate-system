...
class SecurityAddRecord(tk.Frame):
    def __init__(self, master, security_id, security_name, navigate_to=None, navigate_to_login=None, *args, **kwargs):
    ...
    def build_form(self):
        ...
        # Hide owner_id initially if not student/staff
    ...
    def save_record(self):
        ...
        if not data["entry_date"] or not data["entry_time"]:
            self.play_warning_sound()
            messagebox.showerror("Error", "Entry date and time are required fields.")
            return
        ...
        try:
            # Validate entry date and time format
        ...
        try:
            record_id = self.firebase_db.add_record(
                data["vehicle_plate"],
                data["owner_identity"],
                data["owner_phone"],
                data["entry_date"],
